...
#ifndef RANER_NET_EPOLL_SERVER_H_
#define RANER_NET_EPOLL_SERVER_H_
...
class EpollServer {
 public:
  ...
  bool HasRegisterRead(int fd) const;
  bool HasRegisterWrite(int fd) const;
  ...
  int64_t timeout_in_us() const { return timeout_in_us_; }
 ...
 protected:
  ...
  struct CBAndEventMask {
    ...
    CBAndEventMask(EpollCallbackInterface *c, int em, int f)
        : cb(c),
          fd(f),
          event_mask(em),
          events_asserted(0),
  ...
  struct EventRecorder {
   public:
    ...
    void Clear() {
      for (int i = 0; i < static_cast<int>(debug_events_.size()); ++i) {
    ...
    friend ostream &operator<<(ostream &os, const EventRecorder &er) {
      for (int i = 0; i < static_cast<int>(er.unregistered_fds_.size()); ++i) {
      ...
      for (int i = 0; i < static_cast<int>(er.debug_events_.size()); ++i) {
...
#endif  // RANER_NET_EPOLL_SERVER_H_
